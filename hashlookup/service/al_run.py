...
class AssemblylineService(ServiceBase):
    ...
    def _get_details(self, sha1: str):
        response = self._session.get(f"{HASHLOOKUP_API}/lookup/sha1/{sha1}").json()
        data = {
            "source": response.get("source", ""),
            "name": response.get("FileName", ""),
            "db": response.get("db", ""),
            "trust": response.get("hashlookup:trust", 50),
            "parents": response.get("parents", [])[:MAX_PARENTS],
    ...
    def execute(self, request: ServiceRequest) -> None:
        info = self.lookup_sha1(request.sha1)
        if not info:
            request.result = Result()
            return

        main_section = ResultMultiSection("Hashlookup Results")
